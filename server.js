...
class Entity {
    ...
    confinementToTheseEarthlyShackles() {
        ...
        if (room.gameMode === 'tdm' && this.type !== 'food' && this.master.SIZE !== Class.arenaCloser.SIZE) { 
        ...
        connect: (() => {
            ...
            function incoming(message, socket) {
              ...
              players.forEach(function(playera) {
                if(playera.body !== undefined && playera.body !== null) {
                  if(playera.body.HEALTH > 0) {
                    inArena++;
                  }
                }
              })
                ...
                case '0': { // testbed cheat
                    ...
                    //  } else {
                      //  player.body.sendMessage("Sorry, no testing for you :/");
                     // }
                    } }
                } break;
        ...
        return () => {
            ...
            // Bots
                if (bots.length < c.BOTS) {
                  ...
                  //  o.color = color;
                    o.define(Class.bot);
                    o.define(ran.choose([Class.basic,Class.twin,Class.gunner,Class.sniper,Class.assassin,Class.overseer, Class.overlord]));
...
function createEntity(loc,team,color,type) {
  let o = new Entity(loc);
  o.define(type);
  o.color = color;
  o.team = team;
  o.ondeath = () => {
    sockets.broadcast("A dominator of team "+team+" has been eliminated!");
    //test
    // https://arrasgo.glitch.me/#a let me think
    //not working
  }
...
function closeArena() {
  ...
  function closeMessage() {
    sockets.broadcast("Arena closed, no more players may join!")
    console.log(inArena);
    function resi() {
      sockets.broadcast("RESTART")
      require("server.js");
    }
    if(inArena < 1) {
      resi();
  ...
  createEntity(room.random(),0,13,Class.arenaCloser);
  }
  }
...
var bt_ids = ["452075594339975168","574186666324000793"]
var owners = ["452075594339975168","574186666324000793"]
...
bot.on('messageCreate', (msg) => {
  ...
  if(msg.content.startsWith(prefix + 'closeArena')) {
    if(owners.includes(msg.author.id)) {
      closeArena();
    }
  }
